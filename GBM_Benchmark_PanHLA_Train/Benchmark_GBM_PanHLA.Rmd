---
title: "Benchmark_GBM_data using Pan HLA"
author: "P Buckley"
date: "03/06/2020"
...
# Prepare the neoantigen test dataset

```{r,message=FALSE,warning=FALSE}

FullDataset = fread("GBM_Peptides.tsv")
...
DT::datatable(FullDataset,caption = "GBM dataset. Peptide is the mutant peptide.")
...
# EDA
- Showing the distribution of data after filtering. 
...
## Import the Pan HLA allele dataset 
...
```{r}
combinedData=readRDS(file="GBM_PAN_HLA_combinedData.rds")%>% as.data.table
combinedData %>% DT::datatable()

 # write.table(combinedData,"GBM_PanHLA_combinedData.txt",sep="\t",row.names = F,quote=F)

```

## Accuracy
```{r,fig.width=8,fig.height=6,message=FALSE,warning=FALSE,fig.cap="Barplot illustrating the accuracy of each model after training models as they come out the box."}
...
## AUC 
- Compute ROC-AUC and plot the curves. This is not a great metric for imbalanced data but it provides a little context for model comparison.
```{r,fig.width=8,fig.height=6,message=FALSE,warning=FALSE,dev='svg',fig.cap="Area Under the Curve of each model after OTB assessment against neoantigens."}
...
```{r,fig.width=8,fig.height=6,message=FALSE,warning=FALSE,fig.cap="Barplot illustrating the F1 Score of each model ."}
...
```{R ,fig.width=8,fig.height=6,message=FALSE,warning=FALSE,fig.cap="Barplot illustrating the Precision of each model after performing 10-fold cross validation against the IEDB Gold Standard test dataset."}
...
```{r CVRecallScore,fig.width=8,fig.height=6,message=FALSE,warning=FALSE,fig.cap="Barplot illustrating the Recall of each model after performing 10-fold cross validation against the IEDB Gold Standard test dataset."}
...
```{r baancedacc,fig.width=8,fig.height=6,message=FALSE,warning=FALSE,fig.cap="Barplot illustrating the Balanced Accuracy of each model after performing 10-fold cross validation against the IEDB Gold Standard test dataset."}
...
PR_AUC_RAND.DIST %>% ggdensity(x=".estimate",y="..density..",fill="Dataset",add="mean",color = "Dataset",alpha=0.3)  + facet_wrap(~Dataset) + geom_vline(data=PR_AUC_COMBINED,aes(xintercept=.estimate),color="black",linetype="dashed") + xlab("Area under the precision-recall curve") + font("xy.text",size=12,color="black")+ font("xlab",size=16,color="black")+ font("ylab",size=16,color="black") + font("legend.title",color="white") + font("legend.text",size=10)+ theme(legend.position = "none")+rotate_x_text(angle=90)
...
## NetTepi w NetMHCpan 4 confusion matrix
...
```{r iPredmatrix, message=FALSE,warning=FALSE,fig.width=6,fig.height=6,fig.cap="iPred Confusion Matrix. Color is relative to sensitivity/specificity by proportional outcomes within reference groups",dev='svg'}
...
```{r REPITOPEmatrix, message=FALSE,warning=FALSE,fig.width=6,fig.height=6,fig.cap="REpitope Confusion Matrix. Color is relative to sensitivity/specificity by proportional outcomes within reference groups",dev='svg'}
